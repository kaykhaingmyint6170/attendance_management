                    ...
                    <table>
                        ...
                        <tbody>
                            <tr v-for="(rec, i) in records" :key="rec.id" :style="{ animationDelay: i * 0.05 + 's' }">
                                <td><div class="user-cell"><div class="user-avatar">{{ rec.user?.name?.charAt(0) }}</div><span class="font-semibold">{{ rec.user?.name }}</span></div></td>
                                <td>{{ rec.date }}</td><td>{{ formatTime(rec.check_in_at) }}</td><td>{{ formatTime(rec.check_out_at) || '—' }}</td>
...
export default {
    ...
    setup() {
        ...
        const filters = reactive({ date_from: '', date_to: '', user_id: '' });
        function formatTime(dt) { if (!dt) return null; return new Date(dt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }); }
        async function loadPage(page) { loading.value = true; try { const params = new URLSearchParams({ page }); if (filters.date_from) params.append('date_from', filters.date_from); if (filters.date_to) params.append('date_to', filters.date_to); if (filters.user_id) params.append('user_id', filters.user_id); const { data } = await api.get(`/attendance?${params}`); records.value = data.data; pagination.value = { current_page: data.current_page, last_page: data.last_page, next_page_url: data.next_page_url, prev_page_url: data.prev_page_url }; } catch (e) {} finally { loading.value = false; } }
        function applyFilters() { loadPage(1); }
        function resetFilters() { filters.date_from = ''; filters.date_to = ''; filters.user_id = ''; loadPage(1); }
        onMounted(async () => { try { const res = await api.get('/users'); users.value = res.data; } catch (e) {} loadPage(1); });
        return { loading, records, users, pagination, filters, formatTime, loadPage, applyFilters, resetFilters };
    }
};
</script>
...
.table-card { animation: fadeInUp 0.5s ease-out 0.15s both; }
...
:global(.dark) .filter-field label { color: #64748b; }
